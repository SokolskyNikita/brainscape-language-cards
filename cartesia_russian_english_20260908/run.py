"""Resumable answer-only audio rollout. No card text writes or deletions."""
...
assert P==21919521 and len(M['items'])==10000
...
def upload(i):
 ...
 for attempt in range(4):
  try:
   result=C.attach_audio(P,i['deckId'],i['cardId'],answer_file=audiofile(i));break
 ...
 progress('uploaded')
def check(i,card):
 if card['question']!=i['question'] or card['answer']!=i['answer'] or card.get('qSoundUrl')!=i['qSoundUrl']:
  raise RuntimeError(f'Text or question audio mismatch on {i["cardId"]}')
 url=card.get('aSoundUrl')
...
# Reuse the 20 user-approved recordings when source text still matches exactly.
T=R.parent/'cartesia_test_20260908';sample=json.loads((T/'manifest.json').read_text())
assert sample['generation_config']==M['generation_config'] and sample['voiceId']==M['voice']
by_source={x['sourceCardId']:x for x in sample['items']}
for i in M['items']:
 s=by_source.get(i['cardId'])
 if s and s['transcript']==i['transcript'] and not audiofile(i).exists():audiofile(i).write_bytes((T/s['file']).read_bytes())
def verify_deck(deck,items):
 did=deck["deckId"];done=R/"verified"/f"{did}.json"
 print("VERIFYING DECK",did,flush=True)
 for attempt in range(8):
  live=C.list_cards(P,did);byid={c['cardId']:c for c in live}
 ...
 atomic(done,{'deckId':did,'name':deck['name'],'cards':500,'verifiedAt':time.time(),'textPreserved':True,'questionAudioPreserved':True,'answerBytesMatched':500})
...
with concurrent.futures.ThreadPoolExecutor(max_workers=1) as verifier:
 ...
 for deck in M['decks']:
  ...
  live=C.list_cards(P,did);live_byid={c['cardId']:c for c in live}
...
print('COMPLETE: 10,000 answer audio files verified; 20 decks; all text and question audio preserved.',flush=True)

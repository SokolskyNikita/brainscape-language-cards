...
BASE=ROOT/'russian_english_pass3_20260908';OUT=BASE/'cloud_sync';OUT.mkdir(exist_ok=True)
...
def main():
 ...
 for p in sorted((ROOT/'russian_english').glob('deck_*.csv')):
  deck=int(p.stem[5:]);want=cards_from_path(p);old=cards_from_path(BASE/'originals'/p.name);remote=client.list_cards(21919521,deck)
  ...
  for b,c in changes:client.update_card(21919521,deck,b['cardId'],**card_write_payload(c))
  after=client.list_cards(21919521,deck)

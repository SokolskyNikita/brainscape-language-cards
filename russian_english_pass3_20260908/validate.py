...
STOP_EN.add("into")  # Basic preposition; not supporting content vocabulary.
from review import ROOT,BASE,cards_from_path
paths=sorted((ROOT/'russian_english').glob('deck_*.csv')); decks=[cards_from_path(p) for p in paths];first={};issues=[];errors=[];changed=0
for band,cards in enumerate(decks):
 for c in cards:
  for lang,key in [('ru','qMdBody'),('en','aMdBody')]:
...
for band,(p,cards) in enumerate(zip(paths,decks)):
 ...
 for row,c in enumerate(cards,1):
  ...
  flags={}
  for lang,key,ex in [('ru','qMdBody','qMdFootnote'),('en','aMdBody','aMdFootnote')]:
